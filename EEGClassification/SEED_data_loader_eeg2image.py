...
<<<<<<< HEAD
        eeg = eeg[:, self.time_low:self.time_high] # Cut EEG from (62, 800) => (62, 600)
=======
...
<<<<<<< HEAD
        ...
        if (self.mode == "train"):
            eeg = eeg.numpy()
            ## If EEG is not downsampled, uncomment below line. For downsample eeg, size is already (128, 128)
            eeg = np.array([self.augment(samples=eeg[i], sample_rate= 500 ) for i in range(eeg.shape[0])])
=======
        # Augment eeg using audiomentations (must convert to numpy first)
        if (self.mode == "train"):
            eeg = eeg.numpy()
...
<<<<<<< HEAD
        eeg_heatmap = F.interpolate(grayscale_images, size=(416, 500), mode='bilinear').squeeze(0).squeeze(0)
=======
