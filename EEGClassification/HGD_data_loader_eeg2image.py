...
class EEG2Image_Augment_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            raise ValueError()
        eeg, label = [self.eeg_dataset[dataset_idx][key] for key in ['eeg', 'label']]

        ## If EEG is not downsampled, uncomment below line. For downsample eeg, size is already (128, 128)
        eeg = eeg[:, self.time_low:self.time_high] # (channels, time_steps) => (128, 440)
        ## 
        ...
        if (self.mode == "train"):
            # eeg = eeg.numpy()
            ## If EEG is not downsampled, uncomment below line. For downsample eeg, size is already (128, 128)
            eeg = np.array([self.augment(samples=eeg[i], sample_rate=250) for i in range(eeg.shape[0])])
            ##
        ...
        eeg_heatmap = F.interpolate(grayscale_images, size=(2*128, 250), mode='bilinear').squeeze(0).squeeze(0)
        ##
        ...
        # edges = cv2.adaptiveThreshold(eeg_heatmap, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 5, 2)
        edges = cv2.Canny(eeg_heatmap, 50,115 )
